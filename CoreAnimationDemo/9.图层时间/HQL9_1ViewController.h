...
/*
 ...
 CAMediaTiming 协议定义了在一段动画内用来控制逝去时间的属性的集合，
 CALayer 和 CAAnimation 都实现了这个协议，所以时间可以被任意基于一个图层或者一段动画的类控制。
 
 ## 持续和重复
 
 duration 属性：对将要进行的动画的“一次迭代”指定了时间。
 duration 是一个 CFTimeInterval 的类型（类似于 NSTimeInterval 的一种双精度浮点类型）。
 
 repeatCount 属性：动画重复的迭代次数。
